...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.use_balanced_weights:
            classes_weights_path = os.path.join(args.dataset, args.dataset+'_classes_weights.npy')
            if os.path.isfile(classes_weights_path):
                weight = np.load(classes_weights_path)
            else:
                weight = calculate_weights_labels(args.dataset, self.train_loader, self.num_class)
...
def main():
    ...
    parser.add_argument("--checkdir", type=str, default=None, help="checkpoint directory")
